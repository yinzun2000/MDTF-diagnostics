...
CalTFSAFS  <-  function(ef.i,pre.am.i,pre.pm.i,
                     pre.thr=.1,n.bin=10){
    #select available dates
    #select days with positive ctp.i, low pre.am, and pre.pm
    idx.nona  <-  seq(1,length(ef.i))[!is.na(ef.i) &
                                      !is.na(pre.am.i) &
                                      !is.na(pre.pm.i) &
                                      pre.am.i < pre.thr]

    ef.t  <-  ef.i[idx.nona]
    pre.t <-  pre.pm.i[idx.nona]
    pev.t <-  pre.t
    ...
    q.ef  <-  as.numeric(quantile(ef.t,probs=seq(0,1,length.out=n.bin+1)))
...
#    if (length(r.o[is.na(r.o)]) <= 2)
#        tfs.o <- as.numeric(lm(r.o~q.ef.t)$coefficients[2])*sd(ef.t)
#    if (length(e.o[is.na(e.o)]) <= 5)
#        afs.o <- as.numeric(lm(e.o~q.ef.t)$coefficients[2])*sd(ef.t)
    ...
    return(list(tfs=sum.tfs*sd(ef.t),afs=sum.afs*sd(ef.t)))
}
