...
CircRemove  <-  function(v.i,ts.i){
    dates.t <-  format(ts.i,'%m-%d')
    dates.u <-  unique(dates.t)
    len.t <-  length(dates.t)

    n.dat <-  length(dates.u)
    for (nt in 1:n.dat)
    {
        ind.t <-  seq(1,len.t)[dates.t == dates.u[nt]]
        v.o <-  detrend(v.i)
        m.tmp <-  mean(v.o[ind.t],na.rm=T)
        v.o[ind.t]  <-  v.o[ind.t] - m.tmp
    }
    return(v.o)
}
